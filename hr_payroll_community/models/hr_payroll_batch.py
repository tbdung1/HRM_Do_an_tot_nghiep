...
class HrPayrollBatch(models.Model):
    ...
    email_sent_count = fields.Integer(
        string='Email Đã Gửi',
        readonly=True,
        help="Số lượng email đã gửi thành công"
    )
    ...
    notes = fields.Text(
        string='Ghi Chú',
        help="Ghi chú về bảng lương này"
    )
    ...
    def action_generate_payslips(self):
        ...
        for batch in self:
            ...
            if not employees:
                batch.state = 'draft'
                raise UserError(_("Không tìm thấy nhân viên có hợp đồng đang chạy trong kỳ đã chọn!"))
    ...
    def action_compute_payslips(self):
        ...
        for batch in self:
            ...
            batch.message_post(body=_("Đã tính toán %d phiếu lương") % len(draft_payslips))
    ...
    def action_mark_done(self):
        ...
        for batch in self:
            ...
            unverified = batch.payslip_ids.filtered(lambda p: p.state not in ['done', 'cancel'])
            if unverified:
                raise UserError(_(
    ...
    def _get_or_create_email_template(self):
        ...
        if not template:
            ...
            template_vals = {
                ...
                'subject': 'Phiếu lương tháng {{ object.date_from.strftime("%m/%Y") }}',
                'email_from': '{{ user.email_formatted }}',
                'email_to': '{{ object.employee_id.work_email }}',
                'body_html': '''
                    <p>Kính gửi {{ object.employee_id.name }},</p>
                    
                    <p>Vui lòng xem phiếu lương đính kèm cho kỳ:</p>
                    <p><strong>Từ ngày:</strong> {{ object.date_from.strftime("%d/%m/%Y") }}<br/>
                    <strong>Đến ngày:</strong> {{ object.date_to.strftime("%d/%m/%Y") }}</p>
                    
                    <p><strong>Tổng tiền:</strong> {{ '{:,.0f}'.format(object.total_amount) }} VND</p>
